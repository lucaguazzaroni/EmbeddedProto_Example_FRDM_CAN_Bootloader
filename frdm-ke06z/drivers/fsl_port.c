...
void PORT_SetPinSelect(port_module_t module, port_pin_select_t pin)
{
    ...
    uint32_t shift = (uint32_t)module & ~(PORT_PINSEL_REG_OFFSET | (PORT_MODULEPS_BITWIDTH_MASK << PORT_MODULEPS_BITWIDTH_OFFSET));

    if (module > kPORT_SWDE)
    {
        if (((uint32_t)module & PORT_PINSEL_REG_OFFSET) == 0U)
        {
            pinSelReg = SIM->PINSEL0 & ~ (uint32_t)(((1UL << bitwidth) - 1U) << shift);
            SIM->PINSEL0 = pinSelReg | ((uint32_t)pin << shift);
        }
        else
        {
            pinSelReg = SIM->PINSEL1 & ~ (uint32_t)(((1UL << bitwidth) - 1U) << shift);
...
void PORT_SetFilterSelect(PORT_Type *base, port_filter_pin_t port, port_filter_select_t filter)
{
    ...
    if ((uint32_t)port < PORT_FILTER_REG_OFFSET)
    {
    ...
    {
        temp -= PORT_FILTER_REG_OFFSET;
        /* Clear the filed to zero. */
        fltReg = base->IOFLT1 & ~((uint32_t)FSL_PORT_FILTER_SELECT_BITMASK << (uint32_t)temp);
        /* Set the filter selection. */
        base->IOFLT1 = ((uint32_t)filter << temp) | fltReg;         
    }
